...
from bitrootlib.lib import (config, util, blocks, check, backend, database, transaction, exceptions)
from bitrootlib.lib.backend.indexd import extract_addresses, extract_addresses_from_txlist

from bitrootlib.test.fixtures.params import DEFAULT_PARAMS as DP
from bitrootlib.test.fixtures.scenarios import UNITTEST_FIXTURE, INTEGRATION_SCENARIOS, standard_scenarios_params
...
def init_database(sqlfile, dbfile, options=None):
    ...
    server.initialise(
        ...
        testnet=True,
        verbose=True,
        console_logfilter=os.environ.get('BITROOT_LOGGING', None),
        **kwargs)
...
def run_scenario(scenario):
    ...
    server.initialise(database_file=':memory:', testnet=True, **BITROOTD_OPTIONS)
    ...
    for tx in scenario:
        if tx[0] != 'create_next_block':
            ...
            with MockProtocolChangesContext(**(mock_protocol_changes or {})):
                module = sys.modules['bitrootlib.lib.messages.{}'.format(tx[0])]
...
def check_outputs(tx_name, method, inputs, outputs, error, records, comment, mock_protocol_changes, server_db):
    ...
    try:
        tested_module = sys.modules['bitrootlib.lib.{}'.format(tx_name)]
    except KeyError:    # TODO: hack
        tested_module = sys.modules['bitrootlib.lib.messages.{}'.format(tx_name)]
...
def reparse(testnet=True):
    ...
    data_dir = appdirs.user_data_dir(appauthor=config.BITROOT_NAME, appname=config.APP_NAME, roaming=True)
...
class MockProtocolChangesContext(object):
    def __init__(self, **kwargs):
        from bitrootlib.test.conftest import MOCK_PROTOCOL_CHANGES
