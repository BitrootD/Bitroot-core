...
from bitrootlib.lib import util, backend
...
def test_search_raw_transactions_unconfirmed(server_db):
    ...
    # create send
    v = int(100 * 1e8)
    send1hex = util.api('create_send', {'source': ADDR[0], 'destination': ADDR[1], 'asset': .BITROOT', 'quantity': v})
    ...
    # create send
    v = int(100 * 1e8)
    send2hex = util.api('create_send', {'source': ADDR[0], 'destination': ADDR[1], 'asset': .BITROOT', 'quantity': v})
