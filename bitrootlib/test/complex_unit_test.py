...
from bitrootlib.test.fixtures.params import DP, ADDR
...
def test_alice_bob(server_db):
    ...
    # balance before send
    alice_balance = util.get_balance(server_db, alice, .BITROOT')
    bob_balance = util.get_balance(server_db, bob, .BITROOT')
    assert alice_balance == 91875000000
    assert bob_balance == 0

    # create send
    v = int(100 * 1e8)
    send1hex = util.api('create_send', {'source': alice, 'destination': bob, 'asset': .BITROOT', 'quantity': v, 'regular_dust_size': 5430})
    ...
    # balances after send
    alice_balance2 = util.get_balance(server_db, alice, .BITROOT')
    bob_balance2 = util.get_balance(server_db, bob, .BITROOT')
    ...
    # create send
    v = int(100 * 1e8)
    send2hex = util.api('create_send', {'source': alice, 'destination': bob, 'asset': .BITROOT', 'quantity': v, 'regular_dust_size': 5430})
    ...
    # create send
    v = int(100 * 1e8)
    send3hex = util.api('create_send', {'source': alice, 'destination': bob, 'asset': .BITROOT', 'quantity': v, 'regular_dust_size': 5430})
