...
from bitrootlib.lib import exceptions
from bitrootlib.lib import script
from bitrootlib.lib.messages import issuance
from bitrootlib.lib.api import APIError
from bitrootlib.lib.util import (DebitError, CreditError, QuantityError, RPCError)
from fractions import Fraction
from bitrootlib.lib import address
...
UNITTEST_VECTOR = {
    ...
    'broadcast': {
        'validate': [{
            ...
            'mock_protocol_changes': {'enhanced_sends': True, 'options_require_memo': True},
            'in': (ADDR[5], 1588000000, 1, DP['fee_multiplier'], 'OPTIONS.BITROOT', DP['default_block_index']),
    ...
    'burn': {
        ...
        'compose': [{
            ...
            'out': ('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 62000000)], None)
        }, {
            ...
            'out': ('1_mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc_mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns_2', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 50000000)], None)
        }, {
            'comment': 'p2sh',
            'in': (P2SH_ADDR[0], int(DP['burn_quantity'] / 2)),
            'out': (P2SH_ADDR[0], [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 31000000)], None)
        }],
        'parse': [{
            'in': ({'block_index': DP['default_block_index'], 'destination': 'mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 'fee': 10000, 'block_time': 155409000, 'supported': 1, 'btc_amount': 62000000, 'data': b'', 'source': 'mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', 'tx_index': 502, 'tx_hash': 'db6d9052b576d973196363e11163d492f50926c2f1d1efd67b3d999817b0d04d', 'block_hash': DP['default_block_hash']},),
            'records': [
                {'table': 'burns', 'values': {
                    'block_index': DP['default_block_index'],
                    'burned': 62000000,
    ...
    'destroy': {
        ...
        #}],
        'compose': [{
            'in': (ADDR[0], .BITROOT', 1, bytes(9999999)),
    ...
    'send': {
        'validate': [{
            'in': (ADDR[0], ADDR[1], .BITROOT', DP['quantity'], 1),
            'out': ([])
        }, {
            'in': (ADDR[0], P2SH_ADDR[0], .BITROOT', DP['quantity'], 1),
            'out': ([])
        }, {
            'in': (P2SH_ADDR[0], ADDR[1], .BITROOT', DP['quantity'], 1),
            'out': ([])
        }, {
            'in': (ADDR[0], ADDR[1], 'BTC', DP['quantity'], 1),
            'out': (['cannot send bitcoins'])
        }, {
            'in': (ADDR[0], ADDR[1], .BITROOT', DP['quantity'] / 3, 1),
            'out': (['quantity must be in satoshis'])
        }, {
            'in': (ADDR[0], ADDR[1], .BITROOT', -1 * DP['quantity'], 1),
            'out': (['negative quantity'])
        }, {
            'in': (ADDR[0], MULTISIGADDR[0], .BITROOT', DP['quantity'], 1),
            'out': ([])
        }, {
            ...
            'in': (ADDR[0], ADDR[6], .BITROOT', DP['quantity'], 1),
            'out': (['destination requires memo'])
        }],
        'compose': [{
            'in': (ADDR[0], ADDR[1], .BITROOT', DP['small']),
            ...
            'in': (P2SH_ADDR[0], ADDR[1], .BITROOT', DP['small']),
            ...
            'in': (ADDR[0], P2SH_ADDR[0], .BITROOT', DP['small']),
            ...
            'in': (ADDR[0], ADDR[1], .BITROOT', DP['quantity'] * 10000000),
            'error': (exceptions.ComposeError, 'insufficient funds')
        }, {
            'in': (ADDR[0], ADDR[1], .BITROOT', DP['quantity'] / 3),
            'error': (exceptions.ComposeError, 'quantity must be an int (in satoshi)')
        }, {
            'in': (ADDR[0], MULTISIGADDR[0], .BITROOT', DP['quantity']),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc',
            ...
            'in': (ADDR[0], ADDR[6], .BITROOT', 100000000),
            ...
            'in': ({'source': ADDR[0], 'destination': ADDR[6], 'asset': .BITROOT', 'quantity': 100000000, 'memo': '12345', 'use_enhanced_send': True}),
            ...
            'in': ({'source': ADDR[0], 'destination': P2WPKH_ADDR[0], 'asset': .BITROOT', 'quantity': 100000, 'memo': 'segwit', 'use_enhanced_send': True}),
            ...
            'in': ({'source': ADDR[0], 'destination': [ADDR[1], ADDR[2]], 'asset': [.BITROOT', .BITROOT'], 'quantity': [100000000, 100000000], 'memo': '12345', 'use_enhanced_send': True}),
    ...
    'issuance': {
        'validate': [{
            ...
            'out': (0, 0.0, ['cannot issue BTC or.BITROOT'], 50000000, '', True, False, None)
        }, {
            'in': (ADDR[0], None, .BITROOT', 1000, True, False, None, None, '', None, None, DP['default_block_index']),
            'out': (0, 0.0, ['cannot issue BTC or.BITROOT'], 50000000, '', True, False, None)
        }, {
    ...
    'dividend': {
        'validate': [{
            'in': (ADDR[0], DP['quantity'] * 1000, 'DIVISIBLE', .BITROOT', DP['default_block_index']),
            'out': (1200000000000,
                    [
            ...
            'in': (ADDR[0], DP['quantity'], 'BTC', .BITROOT', DP['default_block_index']),
            'out': (None, None, ['cannot pay dividends to holders of BTC', 'no such asset, BTC.'], 0)
        }, {
            'comment': 'cannot pay dividends to holders of.BITROOT',
            'in': (ADDR[0], DP['quantity'], .BITROOT', .BITROOT', DP['default_block_index']),
            'out': (None, None, ['cannot pay dividends to holders of.BITROOT', 'no such asset,.BITROOT.'], 0)
        }, {
            'comment': 'no such asset, NOASSET',
            'in': (ADDR[0], DP['quantity'], 'NOASSET', .BITROOT', DP['default_block_index']),
            'out': (None, None, ['no such asset, NOASSET.'], 0)
        }, {
            'comment': 'non‐positive quantity per unit',
            'in': (ADDR[0], 0, 'DIVISIBLE', .BITROOT', DP['default_block_index']),
            'out': (0,
                    [
            ...
            'in': (ADDR[1], DP['quantity'], 'DIVISIBLE', .BITROOT', DP['default_block_index']),
            'out': (99900000000,
                    [
                    ...
                    ['only issuer can pay dividends', 'insufficient funds .BITROOT)'],
                    0)
        }, {
            'in': (ADDR[0], DP['quantity'], 'DIVISIBLE', 'NOASSET', DP['default_block_index']),
            'out': (None, None, ['no such dividend asset, NOASSET.'], 0)
        }, {
            'in': (ADDR[0], 8359090909, 'DIVISIBLE', .BITROOT', DP['default_block_index']),
            'out': (100309090908,
                    [
        ...
        'compose': [{
            'in': (ADDR[0], DP['quantity'], 'DIVISIBLE', .BITROOT'),
    ...
    'order': {
        'validate': [{
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'], .BITROOT', DP['quantity'], 2000, 0, DP['default_block_index']),
            'out': ([])
        }, {
            'in': (P2SH_ADDR[0], 'DIVISIBLE', DP['quantity'], .BITROOT', DP['quantity'], 2000, 0, DP['default_block_index']),
            'out': ([])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'], .BITROOT', DP['quantity'], 2000, 0.5, DP['default_block_index']),
            ...
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'] / 3, .BITROOT', DP['quantity'], 2000, 0, DP['default_block_index']),
            'out': (['give_quantity must be in satoshis'])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'], .BITROOT', DP['quantity'] / 3, 2000, 0, DP['default_block_index']),
            'out': (['get_quantity must be in satoshis'])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'], .BITROOT', DP['quantity'], 1.5, 0, DP['default_block_index']),
            'out': (['expiration must be expressed as an integer block delta'])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', -DP['quantity'], .BITROOT', -DP['quantity'], -2000, -10000, DP['default_block_index']),
            'out': (['non‐positive give quantity', 'non‐positive get quantity', 'negative fee_required', 'negative expiration'])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', 0, .BITROOT', DP['quantity'], 2000, 0, DP['default_block_index']),
            ...
            'in': (ADDR[0], 'DIVISIBLE', 2**63 + 10, .BITROOT', DP['quantity'], 4 * 2016 + 10, 0, DP['default_block_index']),
            'out': (['integer overflow', 'expiration overflow'])
        }],
        'compose': [{
            'in': (ADDR[0], 'BTC', DP['small'], .BITROOT', DP['small'] * 2, DP['expiration'], 0),
            ...
            'in': (P2SH_ADDR[0], 'BTC', DP['small'], .BITROOT', DP['small'] * 2, DP['expiration'], 0),
            'out': (P2SH_ADDR[0], [], b'\x00\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\xfa\xf0\x80\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x05\xf5\xe1\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x00')
        }, {
            'in': (ADDR[0], .BITROOT', round(DP['small'] * 2.1), 'BTC', DP['small'], DP['expiration'], DP['fee_required']),
            ...
            'in': (MULTISIGADDR[0], 'BTC', DP['small'], .BITROOT', DP['small'] * 2, DP['expiration'], 0),
            'out': ('1_mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc_mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns_2', [], b'\x00\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\xfa\xf0\x80\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x05\xf5\xe1\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x00')
        }, {
            'in': (MULTISIGADDR[0], .BITROOT', round(DP['small'] * 2.1), 'BTC', DP['small'], DP['expiration'], DP['fee_required']),
            'out': ('1_mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc_mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns_2', [], b'\x00\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x06B,@\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\xfa\xf0\x80\x00\n\x00\x00\x00\x00\x00\r\xbb\xa0')
        }, {
            'in': (ADDR[0], 'MAXI', 2**63 - 1, .BITROOT', DP['quantity'], DP['expiration'], DP['fee_required']),
            ...
            'in': (ADDR[0], 'MAXI', 2**63 - 1, .BITROOT', DP['quantity'], DP['expiration'], 2 ** 63),
            'error': (exceptions.ComposeError, "['integer overflow']")
        }, {
            'in': (ADDR[0], 'MAXI', 2**63, .BITROOT', DP['quantity'], DP['expiration'], DP['fee_required']),
            'error': (exceptions.ComposeError, 'insufficient funds')
        }, {
            'comment': 'give subasset',
            'in': (ADDR[0], 'PARENT.already.issued', 100000000, .BITROOT', DP['small'], DP['expiration'], DP['fee_required']),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc', [],
            ...
            'in': (ADDR[0], .BITROOT', DP['small'], 'PARENT.already.issued', 100000000, DP['expiration'], DP['fee_required']),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc', [],
        ...
        'parse': [{
            ...
            'records': [
                ...
                {'table': 'debits', 'values': {
                    'action': 'open order',
                    'address': '1_mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc_mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns_2',
                    'asset': .BITROOT', 'quantity': 105000000,
                    'block_index': DP['default_block_index'],
                    'event': 'db6d9052b576d973196363e11163d492f50926c2f1d1efd67b3d999817b0d04d',
                }}
            ]
    ...
    'sweep': {
        'validate': [
            ...
            },
            {
                ...
                'out': ['insufficient.BITROOT balance for sweep. Need 0.5.BITROOT for antispam fee']
            },
            {
        ...
        'compose': [
            ...
            },
            {
                ...
                'error': (exceptions.ComposeError, "['insufficient.BITROOT balance for sweep. Need 0.5.BITROOT for antispam fee']")
            }
        ],
        'unpack': [
            {
    ...
    'dispenser': {
        'validate': [
            {
                'in': (ADDR[0], config.BITROOT, 100, 100, 100, 0, None, DP['burn_start']),
                'out': (1, None)
            },
            {
                'in': (ADDR[0], config.BITROOT, 200, 100, 100, 0, None, DP['burn_start']),
            ...
            {
                'in': (ADDR[0], config.BITROOT, 100, 100, 100, 5, None, DP['burn_start']),
            ...
            {
                'in': (ADDR[5], config.BITROOT, 100, 100, 120, 0, None, DP['burn_start']),
                'out': (None, ['address has a dispenser already opened for asset %s with a different mainchainrate' % config.BITROOT])
            },
            {
                'in': (ADDR[5], config.BITROOT, 120, 120, 100, 0, None, DP['burn_start']),
                'out': (None, ['address has a dispenser already opened for asset %s with a different give_quantity' % config.BITROOT])
            },
            {
        ...
        'compose': [
            {
                'in': (ADDR[0], config.BITROOT, 100, 100, 100, 0),
            ...
            {
                'in': (ADDR[5], config.BITROOT, 0, 0, 0, 10),
        ...
        'dispense': [
            ...
            {
                'mock_protocol_changes': { 'dispensers': True }, # Same test as above, but with excess BTC, should give out the same amount of.BITROOT
                'in': ({
                    'tx_hash': 'db6d9052b576d973196363e11163d492f50926c2f1d1efd67b3d999817b0d04d',
                    'source': ADDR[0], 'supported': 1, 'block_index': DP['default_block_index'],
                    'fee': 10000, 'block_time': 155409000, 'block_hash': DP['default_block_hash'],
    ...
    'transaction': {
        ...
        'construct': [{
            'in': (('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 62000000)], None), {'encoding': 'multisig', 'exact_fee': 1.0}),
            'error': (exceptions.TransactionError, 'Exact fees must be in satoshis.')
        }, {
            'in': (('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 62000000)], None), {'encoding': 'multisig', 'fee_provided': 1.0}),
            'error': (exceptions.TransactionError, 'Fee provided must be in satoshis.')
        }, {
            'in': (('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 5429)], None), {'encoding': 'singlesig', 'regular_dust_size': DP['regular_dust_size']}),
            ...
            'in': (('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 62000000)], b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x02\xfa\xf0\x80'), {'encoding': 'foobar'}),
            ...
            'in': (('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 62000000)], b'\x00' * 73), {'encoding': 'opreturn'}),
            'error': (exceptions.TransactionError, 'One `OP_RETURN` output per transaction.')
        }, {
            'in': (('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 2**30)], b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x02\xfa\xf0\x80'), {'encoding': 'multisig'}),
            ...
            'in': (('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 62000000)], b'\x00' * 72), {'encoding': 'opreturn'}),
            ...
            'comment': 'burn',
            'in': (('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 62000000)], None), {'encoding': 'multisig'}),
            ...
            'comment': 'burn P2SH',
            'in': ((P2SH_ADDR[0], [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 62000000)], None), {'encoding': 'multisig', 'regular_dust_size': DP['regular_dust_size']}),
            ...
            'in': (('1_mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc_mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns_2', [('mvBitrootXXXXXXXXXXXXXXXXXXXW24Hef', 50000000)], None), {'encoding': 'multisig'}),
    ...
    'api': {
        'get_rows': [{
            ...
            'out': [{'tx_index': 497, 'tx_hash': '1410217c3b1b38ea0b90940f20b874a2375c457b0828de9f9808e4fd63fd54e6', 'block_index': 310496, 'source': 'mnfAHmddVibnZNSkh8DvKaQoiEfNsxjXzH', 'destination': 'mqPCfvqTfYctXMUfmniXeG2nyaN8w6tPmj', 'asset': .BITROOT', 'quantity': 92945878046, 'status': 'valid', 'memo': None, 'memo_hex': None, 'msg_index': 0}]
        }, {
            'comment': 'with memo',
            'in': ('sends', [{'field':'block_index','op':'=','value':'310481'}], 'AND', None, None, None, None, None, 1000, 0, True),
            'out': [{'tx_index': 482, 'tx_hash': 'b00bdf03402d81a2cbdbeac4b0df90cff5ab6bf9688f653383d49fe42b8422a5', 'block_index': 310481, 'source': 'mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc', 'destination': 'mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', 'asset': .BITROOT', 'quantity': 100000000, 'status': 'valid', 'memo': 'hello', 'memo_hex': '68656c6c6f', 'msg_index': 0}]
        }, {
            ...
            'out': [{'tx_index': 483, 'tx_hash': 'c8716524f33646b9af94d6f5e52494ff3b34466497094b1db2ab920e4f79bc34', 'block_index': 310482, 'source': 'mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', 'destination': 'mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc', 'asset': .BITROOT', 'quantity': 100000000, 'status': 'valid', 'memo': '', 'memo_hex': 'fade0001', 'msg_index': 0}]
        }],
    },
    'script': {
        'validate': [{
    ...
    'util': {
        'api': [{
            ...
            'in': ('create_send', {'source': ADDR[0], 'destination': ADDR[1], 'asset': .BITROOT', 'quantity': DP['small'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_send', {'source': P2SH_ADDR[0], 'destination': ADDR[1], 'asset': .BITROOT', 'quantity': DP['small'], 'encoding': 'multisig', 'dust_return_pubkey': False, 'regular_dust_size': DP['regular_dust_size']}),
            'out': ''
        }, {
            ...
            'in': ('create_order', {'source': ADDR[0], 'give_asset': 'BTC', 'give_quantity': DP['small'], 'get_asset': .BITROOT', 'get_quantity': DP['small'] * 2, 'expiration': DP['expiration'], 'fee_required': 0, 'fee_provided': DP['fee_provided'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_order', {'source': ADDR[0], 'give_asset': .BITROOT', 'give_quantity': round(DP['small'] * 2.1), 'get_asset': 'BTC', 'get_quantity': DP['small'], 'expiration': DP['expiration'], 'fee_required': DP['fee_required'], 'encoding': 'multisig'}),
            ...
            'in': ('create_send', {'source': ADDR[0], 'destination': MULTISIGADDR[0], 'asset': .BITROOT', 'quantity': DP['quantity'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_send', {'source': MULTISIGADDR[0], 'destination': ADDR[0], 'asset': .BITROOT', 'quantity': DP['quantity'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_send', {'source': MULTISIGADDR[0], 'destination': MULTISIGADDR[1], 'asset': .BITROOT', 'quantity': DP['quantity'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            ...
            'comment': '1',
            'in': ('create_order', {'source': MULTISIGADDR[0], 'give_asset': 'BTC', 'give_quantity': DP['small'], 'get_asset': .BITROOT', 'get_quantity': DP['small'] * 2, 'expiration': DP['expiration'], 'fee_required': 0, 'fee_provided': DP['fee_provided'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_order', {'source': MULTISIGADDR[0], 'give_asset': .BITROOT', 'give_quantity': round(DP['small'] * 2.1), 'get_asset': 'BTC', 'get_quantity': DP['small'], 'expiration': DP['expiration'], 'fee_required': DP['fee_required'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_dividend', {'source': ADDR[0], 'quantity_per_unit': DP['quantity'], 'asset': 'DIVISIBLE', 'dividend_asset': .BITROOT', 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_dividend', {'source': ADDR[0], 'quantity_per_unit': 1, 'asset': 'NODIVISIBLE', 'dividend_asset': .BITROOT', 'encoding': 'multisig'}),
            'out': ''
        ...
        # CIP 9 enhanced_send tests
            ...
            'in': ('create_send', {'source': ADDR[0], 'destination': ADDR[1], 'asset': .BITROOT', 'quantity': DP['small']}),
            ...
            'in': ('create_send', {'use_enhanced_send': False, 'source': ADDR[0], 'destination': ADDR[1], 'asset': .BITROOT', 'quantity': DP['small']}),
            'out': '01000000'+'01'+'c1d8c075936c3495f6d653c50f73d987f75448d97a750249b1eb83bee71b24ae'+'00000000'+'19'+'76a9144838d8b3588c4c7ba7c1d06f866e9b3739c6303788ac'+'ffffffff'+'03'+'3615000000000000'+'19'+'76a9148d6ae8a3b381663118b4e1eff4cfc7d0954dd6ec88ac'+'0000000000000000'+'1e'+'6a1c2a504df746f83442653dd7ada4dc727a030865749e9fba5aec80c39a'+'2f30ea0b00000000'+'19'+'76a9144838d8b3588c4c7ba7c1d06f866e9b3739c6303788ac'+'00000000'
        }, {
            ...
            'mock_protocol_changes': {'enhanced_sends': True},
            'in': ('create_send', {'memo': 'hello', 'source': ADDR[0], 'destination': ADDR[1], 'asset': .BITROOT', 'quantity': DP['small']}),
            ...
            'mock_protocol_changes': {'enhanced_sends': True},
            'in': ('create_send', {'memo': '0102030405', 'memo_is_hex': True, 'source': ADDR[0], 'destination': ADDR[1], 'asset': .BITROOT', 'quantity': DP['small']}),
            ...
            'mock_protocol_changes': {'enhanced_sends': True, 'options_require_memo': True},
            'in': ('create_send', {'source': ADDR[0], 'destination': ADDR[6], 'asset': .BITROOT', 'quantity': DP['small']}),
            ...
            'mock_protocol_changes': {'enhanced_sends': True, 'options_require_memo': True},
            'in': ('create_send', {'memo': '0102030405', 'memo_is_hex': True, 'source': ADDR[0], 'destination': ADDR[6], 'asset': .BITROOT', 'quantity': DP['small']}),
        ...
        # unpack API method
            ...
            'out': [0, {'asset': .BITROOT', 'quantity': 50000000}]
        }, {
            ...
            'out': [2, {'address': 'mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', 'asset': .BITROOT', 'memo': None, 'quantity': 50000000}]
        }],
        ...
        'generate_asset_id': [{
            ...
            'in': (.BITROOT', DP['default_block_index']),
            'out': 1
        }, {
            'in': ('BCD', 308000),
            'error': (exceptions.AssetNameError, 'too short')
        ...
        {
            'in': (.BITROOT.child1',),
            'error': (exceptions.AssetNameError, "parent asset cannot be.BITROOT")
        },
        {
            'in': ('PARENT.',),
    ...
    'versions.enhanced_send': {
        ...
        {
            'in': (ADDR[0], ADDR[1], .BITROOT', DP['quantity'], None, 1),
            'out': ([])
        }, {
            'in': (ADDR[0], P2SH_ADDR[0], .BITROOT', DP['quantity'], None, 1),
            'out': ([])
        }, {
            'in': (P2SH_ADDR[0], ADDR[1], .BITROOT', DP['quantity'], None, 1),
            'out': ([])
        }, {
            'in': (ADDR[0], ADDR[1], 'BTC', DP['quantity'], None, 1),
            'out': (['cannot send {}'.format(config.BTC)])
        }, {
            'in': (ADDR[0], ADDR[1], .BITROOT', DP['quantity'] / 3, None, 1),
            'out': (['quantity must be in satoshis'])
        }, {
            'in': (ADDR[0], ADDR[1], .BITROOT', -1 * DP['quantity'], None, 1),
            'out': (['negative quantity'])
        }, {
            'in': (ADDR[0], MULTISIGADDR[0], .BITROOT', DP['quantity'], None, 1),
            'out': ([])
        }, {
            ...
            'in': (ADDR[0], ADDR[6], .BITROOT', DP['quantity'], None, 1),
            'out': (['destination requires memo'])
        }, {
        ...
        # ----- tests copied from regular send -----
        {
            'in': (ADDR[0], ADDR[1], .BITROOT', DP['quantity'] * 10000000, None, False),
            'error': (exceptions.ComposeError, 'insufficient funds')
        }, {
            'in': (ADDR[0], ADDR[1], .BITROOT', DP['quantity'] / 3, None, False),
            'error': (exceptions.ComposeError, 'quantity must be an int (in satoshi)')
        }, {
        ...
        # ----- tests specific to enhanced send -----
        {
            ...
            'in': (ADDR[0], ADDR[6], .BITROOT', DP['small'], 'deadbeef', True),
            ...
            'in': (P2WPKH_ADDR[0], ADDR[0], .BITROOT', DP['small'], None, False),
    ...
    'versions.mpma': {
        'unpack': [{
            ...
            'out': ({.BITROOT': [(ADDR[2], DP['quantity']), (ADDR[1], DP['quantity'])]})
        },{
            ...
            'out': ({.BITROOT': [(ADDR[2], DP['quantity'], bytes.fromhex('DEADBEEF'), True), (ADDR[1], DP['quantity'])]})
        },{
            ...
            'out': ({.BITROOT': [(ADDR[2], DP['quantity'], 'DEADBEEF', False), (ADDR[1], DP['quantity'])]})
        },{
            ...
            'out': ({.BITROOT': [(ADDR[3], DP['quantity']), (ADDR[2], DP['quantity']), (ADDR[1], DP['quantity'])]})
        },{
            ...
            'out': ({.BITROOT': [(ADDR[3], DP['quantity'])], 'NODIVISIBLE': [(ADDR[1], 1)], 'DIVISIBLE': [(ADDR[2], DP['quantity'])]})
        },{
            ...
            'out': ({.BITROOT': [(ADDR[2], DP['quantity'], bytes.fromhex('DEADBEEF'), True), (ADDR[1], DP['quantity'], bytes.fromhex('DEADBEEF'), True)]})
        },{
            ...
            'out': ({.BITROOT': [(ADDR[2], DP['quantity'], bytes.fromhex('BEEFDEAD'), True), (ADDR[1], DP['quantity'], bytes.fromhex('DEADBEEF'), True)]})
        },{
            ...
            'out': ({'DIVISIBLE': [(ADDR[1], DP['quantity'])], .BITROOT': [(ADDR[2], DP['quantity'])]})
        },{
        ...
        'validate': [{
            ...
            'in': (ADDR[0], [(.BITROOT', ADDR[1], DP['quantity'])], 1),
            'out': (['send list cannot have only one element'])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], 0.1)], 1),
            'out': (['quantities must be an int (in satoshis) for.BITROOT to {}'.format(ADDR[1])])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], -DP['quantity'])], 1),
            'out': (['negative quantity for.BITROOT to {}'.format(ADDR[1])])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], 0)], 1),
            'out': (['zero quantity for.BITROOT to {}'.format(ADDR[1])])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], config.MAX_INT + 1)], 1),
            'out': (['integer overflow for.BITROOT to {}'.format(ADDR[1])])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', None, DP['quantity'])], 1),
            'out': (['destination is required for.BITROOT'])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), ('BTC', ADDR[1], DP['quantity'])], 1),
            'out': (['cannot send BTC to {}'.format(ADDR[1])])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[6], DP['quantity'])], 1),
            'out': (['destination {} requires memo'.format(ADDR[6])])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], DP['quantity'])], 1),
            'out': ([])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[2], DP['quantity'] + 1)], 1),
            'out': (['cannot specify more than once a destination per asset'])
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[6], DP['quantity'], 'DEADBEEF', True)], 1),
            'out': ([])
        }],
        'compose': [{
            'in': (ADDR[0], [(.BITROOT', ADDR[1], DP['quantity'] * 1000000)], None, None),
            'error': (exceptions.ComposeError, 'insufficient funds for.BITROOT')
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], 0.1)], None, None),
            'error': (exceptions.ComposeError, 'quantities must be an int (in satoshis) for.BITROOT')
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], DP['quantity'] * 10000)], None, None),
            'error': (exceptions.ComposeError, 'insufficient funds for.BITROOT')
        }, {
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], DP['quantity'])], None, None),
            'mock_protocol_changes': {'short_tx_type_id': True},
            ...
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity'], 'DEADBEEF', True), (.BITROOT', ADDR[1], DP['quantity'])], None, None),
            'mock_protocol_changes': {'short_tx_type_id': True},
            ...
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity'], 'DEADBEEF', False), (.BITROOT', ADDR[1], DP['quantity'])], None, None),
            'mock_protocol_changes': {'short_tx_type_id': True},
            ...
            'in': (ADDR[0], [(.BITROOT', ADDR[3], DP['quantity']), (.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], DP['quantity'])], None, None),
            'mock_protocol_changes': {'short_tx_type_id': True},
            ...
            'in': (ADDR[0], [(.BITROOT', ADDR[3], DP['quantity']), ('DIVISIBLE', ADDR[2], DP['quantity']), ('NODIVISIBLE', ADDR[1], 1)], None, None),
            'mock_protocol_changes': {'short_tx_type_id': True},
            ...
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], DP['quantity'])], 'DEADBEEF', True),
            'mock_protocol_changes': {'short_tx_type_id': True},
            ...
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), (.BITROOT', ADDR[1], DP['quantity'])], 'DEADBEEF', False),
            'mock_protocol_changes': {'short_tx_type_id': True},
            ...
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity'], 'BEEFDEAD', True), (.BITROOT', ADDR[1], DP['quantity'])], 'DEADBEEF', True),
            'mock_protocol_changes': {'short_tx_type_id': True},
            ...
            'in': (ADDR[0], [(.BITROOT', ADDR[2], DP['quantity']), ('DIVISIBLE', ADDR[1], DP['quantity'])], None, None),
            'mock_protocol_changes': {'short_tx_type_id': True},
