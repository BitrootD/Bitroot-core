...
from bitrootlib.lib import log
from bitrootlib import server
log.set_logger(logger)

from bitrootlib.test import util_test
from bitrootlib.test.fixtures.vectors import UNITTEST_VECTOR
from bitrootlib.test.fixtures.params import DEFAULT_PARAMS
from bitrootlib.test.fixtures.scenarios import INTEGRATION_SCENARIOS

from bitrootlib.lib import config, util, database, api, script, arc4
...
def setup_logging():
    ...
    log.set_up(log.ROOT_LOGGER, verbose=True, console_logfilter=os.environ.get('BITROOT_LOGGING', None))
...
def init_mock_functions(request, monkeypatch, mock_utxos, rawtransactions_db):
    ...
    monkeypatch.setattr('bitrootlib.lib.transaction.arc4.init_arc4', init_arc4)
    monkeypatch.setattr('bitrootlib.lib.backend.get_unspent_txouts', get_unspent_txouts)
    monkeypatch.setattr('bitrootlib.lib.log.isodt', isodt)
    monkeypatch.setattr('bitrootlib.lib.log.curr_time', curr_time)
    monkeypatch.setattr('bitrootlib.lib.util.date_passed', date_passed)
    monkeypatch.setattr('bitrootlib.lib.api.init_api_access_log', init_api_access_log)
    if hasattr(config, 'PREFIX'):
        monkeypatch.setattr('bitrootlib.lib.config.PREFIX', b'TESTXXXX')
    monkeypatch.setattr('bitrootlib.lib.backend.getrawtransaction', mocked_getrawtransaction)
    monkeypatch.setattr('bitrootlib.lib.backend.getrawtransaction_batch', mocked_getrawtransaction_batch)
    monkeypatch.setattr('bitrootlib.lib.backend.search_raw_transactions', mocked_search_raw_transactions)
    monkeypatch.setattr('bitrootlib.lib.backend.pubkeyhash_to_pubkey', pubkeyhash_to_pubkey)
    monkeypatch.setattr('bitrootlib.lib.backend.multisig_pubkeyhashes_to_pubkeys', multisig_pubkeyhashes_to_pubkeys)
