...
from bitrootlib.lib import check
from bitrootlib.lib import backend
from bitrootlib.lib import database
from bitrootlib.lib import transaction
from bitrootlib.lib import blocks
from bitrootlib.lib import script
from bitrootlib.lib import message_type
from bitrootlib.lib.messages import send
from bitrootlib.lib.messages.versions import enhanced_send
from bitrootlib.lib.messages import order
from bitrootlib.lib.messages import btcpay
from bitrootlib.lib.messages import issuance
from bitrootlib.lib.messages import broadcast
from bitrootlib.lib.messages import bet
from bitrootlib.lib.messages import dividend
from bitrootlib.lib.messages import burn
from bitrootlib.lib.messages import destroy
from bitrootlib.lib.messages import cancel
from bitrootlib.lib.messages import rps
from bitrootlib.lib.messages import rpsresolve
from bitrootlib.lib.messages import sweep
from bitrootlib.lib.messages import dispenser
...
def check_database_state(db, blockcount):
    """Checks {} database to see if is caught up with backend.""".format(config.BITROOT_NAME)
    if util.CURRENT_BLOCK_INDEX + 1 < blockcount:
        raise DatabaseError('{} database is behind backend.'.format(config.BITROOT_NAME))
...
def compose_transaction(db, name, params,
    ...
    compose_method = sys.modules['bitrootlib.lib.messages.{}'.format(name)].compose
    ...
    def run(self):
        ...
        def get_supply(asset):
            if asset == 'BTC':
                ...
                return util.bitroot_supply(self.db)
            else:
                asset = util.resolve_subasset_longname(self.db, asset)
                return util.asset_supply(self.db, asset)

        @dispatcher.add_method
        def get_bitroot_supply():
            logger.warning("Deprecated method: `get_bitroot_supply`")
            return util.bitroot_supply(self.db)
            ...
            for asset in assets:
                ...
                if asset in [config.BTC, config.BITROOT]:
                    if asset == config.BTC:
                        ...
                        supply = util.bitroot_supply(self.db)
