...
from bitrootlib.lib import util
...
class ModuleLoggingFilter(logging.Filter):
    ...
    def ismatch(cls, record, name):
        ...
        Determine if the specified record matches the name, in the same way as original logging.Filter does, ie:
            'bitrootlib.lib' will match 'bitrootlib.lib.check'
        """
        nlen = len(name)
        if nlen == 0:
            return True
...
def set_up(logger, verbose=False, logfile=None, console_logfilter=None):
    ...
    def set_up_file_logging():
        ...
        if os.name == 'nt':
            from bitrootlib.lib import util_windows
...
def log (db, command, category, bindings):
    ...
    if command == 'update':
        ...
        if category == 'credits':
            ...
            logger.info('Bet: {} against {}, by {}, on {}'.format(output(bindings['wager_quantity'], config.BITROOT), output(bindings['counterwager_quantity'], config.BITROOT), bindings['source'], bindings['feed_address']))
            ...
            logger.info('Bet Match: {} for {} against {} for {} on {} at {}{} ({}) [{}]'.format(util.BET_TYPE_NAME[bindings['tx0_bet_type']], output(bindings['forward_quantity'], config.BITROOT), util.BET_TYPE_NAME[bindings['tx1_bet_type']], output(bindings['backward_quantity'], config.BITROOT), bindings['feed_address'], isodt(bindings['deadline']), placeholder, bindings['id'], bindings['status']))
            ...
            logger.info('Burn: {} burned {} for {} ({}) [{}]'.format(bindings['source'], output(bindings['burned'], config.BTC), output(bindings['earned'], config.BITROOT), bindings['tx_hash'], bindings['status']))

        elif category == 'cancels':
            logger.info('Cancel: {} ({}) [{}]'.format(bindings['offer_hash'], bindings['tx_hash'], bindings['status']))

        elif category == 'rps':
            log_message = 'RPS: {} opens game with {} possible moves and a wager of {}'.format(bindings['source'], bindings['possible_moves'], output(bindings['wager'], .BITROOT'))
            logger.info(log_message)

        elif category == 'rps_matches':
            log_message = 'RPS Match: {} is playing a {}-moves game with {} with a wager of {} ({}) [{}]'.format(bindings['tx0_address'], bindings['possible_moves'], bindings['tx1_address'], output(bindings['wager'], .BITROOT'), bindings['id'], bindings['status'])
            logger.info(log_message)
            ...
            if bindings['status'] == 'valid':
                ...
                log_message = 'RPS Resolved: {} is playing {} on a {}-moves game with {} with a wager of {} ({}) [{}]'.format(rps_match['tx0_address'], bindings['move'], rps_match['possible_moves'], rps_match['tx1_address'], output(rps_match['wager'], .BITROOT'), rps_match['id'], rps_match['status'])
            else:
            ...
            if bindings['bet_match_type_id'] == cfd_type_id:
                if bindings['settled']:
                    logger.info('Bet Match Settled: {} credited to the bull, {} credited to the bear, and {} credited to the feed address ({})'.format(output(bindings['bull_credit'], config.BITROOT), output(bindings['bear_credit'], config.BITROOT), output(bindings['fee'], config.BITROOT), bindings['bet_match_id']))
                else:
                    logger.info('Bet Match Force‐Liquidated: {} credited to the bull, {} credited to the bear, and {} credited to the feed address ({})'.format(output(bindings['bull_credit'], config.BITROOT), output(bindings['bear_credit'], config.BITROOT), output(bindings['fee'], config.BITROOT), bindings['bet_match_id']))

            elif bindings['bet_match_type_id'] == equal_type_id:
                logger.info('Bet Match Settled: {} won the pot of {}; {} credited to the feed address ({})'.format(bindings['winner'], output(bindings['escrow_less_fee'], config.BITROOT), output(bindings['fee'], config.BITROOT), bindings['bet_match_id']))
...
def get_asset_info(cursor, asset):
    if asset == config.BTC or asset == config.BITROOT:
