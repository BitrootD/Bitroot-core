...
from bitrootlib.lib import arc4
...
def initialise(db):
    ...
    if not list(cursor):
        ...
        cursor.execute('''INSERT INTO assets VALUES (?,?,?,?)''', ('1', .BITROOT', None, None))
    ...
    while True:
        ...
        if block_index <= block_count:
                ...
                if tx_hash in old_mempool_hashes:
                    for message in old_mempool:
                        if message['tx_hash'] == tx_hash:
                            bitroot_mempool.append((tx_hash, message))

                # If not a supported.BITROOT transaction, skip.
            ...
            with db:
                ...
                for message in bitroot_mempool:
            ...
            logger.getChild('mempool').debug('Refresh mempool: %s.BITROOT txs seen, out of %s total entries (took %ss, next refresh in %ss)' % (
                len(bitroot_mempool), len(raw_mempool),
