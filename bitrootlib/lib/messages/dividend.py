...
from bitrootlib.lib import (config, exceptions, util, message_type)
...
def validate (db, source, quantity_per_unit, asset, dividend_asset, block_index):
    ...
    if asset == config.BITROOT:
        if (not block_index >= 317500) or block_index >= 320000 or config.TESTNET or config.REGTEST:   # Protocol change.
            problems.append('cannot pay dividends to holders of {}'.format(config.BITROOT))
    ...
    if not problems and dividend_asset != config.BTC:
        ...
        if fee:
            balances = list(cursor.execute('''SELECT * FROM balances WHERE (address = ? AND asset = ?)''', (source, config.BITROOT)))
            if not balances or balances[0]['quantity'] < fee:
                problems.append('insufficient funds ({})'.format(config.BITROOT))

    if not problems and dividend_asset == config.BITROOT:
...
def parse (db, tx, message):
    ...
    if status == 'valid':
        ...
        if tx['block_index'] >= 330000 or config.TESTNET or config.REGTEST: # Protocol change.
            util.debit(db, tx['source'], config.BITROOT, fee, action='dividend fee', event=tx['tx_hash'])
