...
from bitrootlib.lib.exceptions import *
...
def validate (db, source, destination, flags, memo, block_index):
    ...
    cursor = db.cursor()
    cursor.execute('''SELECT * FROM balances WHERE (address = ? AND asset = ?)''', (source, .BITROOT'))
    ...
    if antispamfee > 0:
        ...
        if result[0]['quantity'] < total_fee:
            problems.append('insufficient.BITROOT balance for sweep. Need %s.BITROOT for antispam fee' % total_fee)
    else:
        if len(result) == 0:
            problems.append('insufficient.BITROOT balance for sweep. Need %s.BITROOT for antispam fee' % ANTISPAM_FEE_DECIMAL)
...
def parse (db, tx, message):
    ...
    if status == 'valid':
        try:
            ...
            if antispamfee > 0:
                util.debit(db, tx['source'], .BITROOT', total_fee, action='sweep fee', event=tx['tx_hash'])
            else:
                util.debit(db, tx['source'], .BITROOT', fee_paid, action='sweep fee', event=tx['tx_hash'])
        except BalanceError:
            destination, flags, memo_bytes = None, None, None
            status = 'invalid: insufficient balance for antispam fee for sweep'
