...
from bitrootlib.lib import (config, exceptions, util)

"""Burn {} to earn {} during a special period of time.""".format(config.BTC, config.BITROOT)
...
def parse (db, tx, MAINNET_BURNS, message=None):
    ...
    if config.TESTNET or config.REGTEST:
        ...
        if status == 'valid':
            # Calculate quantity of.BITROOT earned. (Maximum 1 BTC in total, ever.)
            cursor = db.cursor()
            ...
            # Credit source address with earned.BITROOT.
            util.credit(db, tx['source'], config.BITROOT, earned, action='burn', event=tx['tx_hash'])
        else:
            burned = 0
            earned = 0
        ...
        util.credit(db, line['source'], config.BITROOT, int(line['earned']), action='burn', event=line['tx_hash'])
