...
written, for example, such that a price of 1.BITROOT means one outcome, 2.BITROOT means
another, etc., which schema should be described in the 'text' field.

fee_fraction: .05.BITROOT means 5%. It may be greater than 1, however; but
    ...
    for bet_match in cursor.fetchall():
        ...
        if bet_match_type_id == cfd_type_id:
            ...
            if bull_credit >= escrow_less_fee or bull_credit <= 0:
                if bull_credit >= escrow_less_fee:
                    ...
                    util.credit(db, bull_address, config.BITROOT, bull_credit, action='bet {}'.format(bet_match_status), event=tx['tx_hash'])
                elif bull_credit <= 0:
                    bull_credit = 0
                    bear_credit = escrow_less_fee
                    bet_match_status = 'settled: liquidated for bear'
                    util.credit(db, bear_address, config.BITROOT, bear_credit, action='bet {}'.format(bet_match_status), event=tx['tx_hash'])

                # Pay fee to feed.
                util.credit(db, bet_match['feed_address'], config.BITROOT, fee, action='feed fee', event=tx['tx_hash'])
        ...
        # Equal[/NotEqual] bet.
            ...
            if value == bet_match['target_value']:
                ...
                util.credit(db, equal_address, config.BITROOT, escrow_less_fee, action='bet {}'.format(bet_match_status), event=tx['tx_hash'])
            else:
                winner = 'NotEqual'
                bet_match_status = 'settled: for notequal'
                util.credit(db, notequal_address, config.BITROOT, escrow_less_fee, action='bet {}'.format(bet_match_status), event=tx['tx_hash'])
